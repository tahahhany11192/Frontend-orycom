document.getElementById('contentForm').addEventListener('submit', async function (e) {
  ...
  try {
    const res = await fetch('https://backend-pfnoxq.fly.dev/api/content/upload', {
      method: 'POST',
      body: formData,
    });
...
async function loadCourses() {
  ...
  const res = await fetch(`https://backend-pfnoxq.fly.dev/api/instructors/${instructorId}/courses`);
...
async function loadInstructorContent() {
  ...
  try {
    const res = await fetch(`https://backend-pfnoxq.fly.dev/api/content/instructor/${instructorId}`);
    ...
    contents.forEach(content => {
      ...
      row.innerHTML = `
        ...
        <td><a href="https://backend-pfnoxq.fly.dev/uploads/${content.file}" target="_blank">View</a></td>
        <td>
...
async function editContent(id) {
  try {
    const res = await fetch(`https://backend-pfnoxq.fly.dev/api/content/${id}`);
...
document.getElementById('contentForm').onsubmit = async (e) => {
  ...
  const url = contentId
    ? `https://backend-pfnoxq.fly.dev/api/content/${contentId}`
    : `https://backend-pfnoxq.fly.dev/api/content/upload`;
